...
class ReplayBuffer:
    ...
    def push (self, state : State, action, reward, next_state: State, done):
        self.buffer.append((state.toTensor(), torch.from_numpy(np.array(action).reshape(4)), torch.tensor(reward), next_state.toTensor(), torch.tensor(done, dtype=torch.float32)))
        # if done:
